<template>
    <div class="mj-switch" :class="{'mj-switch--disabled': disabled}">
        <label :class="{'mj-switch--text-active': !status}">{{notActiveText}}</label>
        <div 
        ...
        <label :class="{'mj-switch--text-active': status}">{{activeText}}</label>
    </div>
</template>
<script lang="ts">
import Vue from 'vue';
...
@Component
export default class MjSwitch extends Vue {
    @Prop({
        validator: function(value: boolean | string | number) {
    ...
    notActiveValue!: number;
    @Prop({type: Number, default: 0})
    activeValue!: number;
    @Prop({type: String, default: "#ee3e3e"})
    notActiveColor!: string;
    @Prop({type: String, default: "#3cf603"})
    activeColor!: string;
    @Prop({
        type: Boolean, 
        default: false
    ...
    created () {
        switch (typeof this.value) {
            case "boolean":
                this.status = this.value as boolean;
                break;
            case "string":
